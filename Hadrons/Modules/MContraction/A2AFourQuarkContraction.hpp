...
void TA2AFourQuarkContraction<FImpl>::setup(void)
{
    ...
    {
        t0Trans_ = strToVec<int>(par().t0Trans);
        nt_ = t0Trans_.size();
...
void TA2AFourQuarkContraction<FImpl>::execute(void)
{
    ...
    {
        LOG(Message) << "Computing 4 quark contraction for: " << getName()
                     << " for time translations: " << par().t0Trans
                     << " and dt = " << dt << std::endl;
        ...
        for (unsigned int t = 0; t < nt_; t++)
        {
            unsigned int t0 = (t0Trans_[t] + dt) % nt;
            LOG(Message) << "Contraction for t = : " << t0 << std::endl;
            mf = mf12[t0];
