...
#define Grid_header "SharedMemoryMpi: "
...
void GlobalSharedMemory::Init(Grid_MPI_Comm comm)
{
  ...
  if ( WorldRank == 0) {
    std::cout << Grid_header " World communicator of size " <<WorldSize << std::endl;  
    std::cout << Grid_header " Node  communicator of size " <<WorldShmSize << std::endl;
  }
...
{
  std::cout << Grid_header "SharedMemoryAllocate "<< bytes<< " shmget implementation "<<std::endl;
  assert(_ShmSetup==1);
  assert(_ShmAlloc==0);
...
void GlobalSharedMemory::SharedMemoryAllocate(uint64_t bytes, int flags)
{
  ...
  std::cout << WorldRank << Grid_header " SharedMemoryMPI.cc acceleratorAllocDevice "<< bytes 
...
void GlobalSharedMemory::SharedMemoryAllocate(uint64_t bytes, int flags)
{
  std::cout << Grid_header "SharedMemoryAllocate "<< bytes<< " MMAP implementation "<< GRID_SHM_PATH <<std::endl;
  assert(_ShmSetup==1);
  assert(_ShmAlloc==0);
  //////////////////////////////////////////////////////////////////////////////////////////////////////////
  // allocate the shared windows for our group
  ...
  for(int r=0;r<WorldShmSize;r++){
    ...
    //    std::cout << Grid_header "Set WorldShmCommBufs["<<r<<"]="<<ptr<< "("<< bytes<< "bytes)"<<std::endl;
  }
  _ShmAlloc=1;
  _ShmAllocBytes  = bytes;
};
...
void GlobalSharedMemory::SharedMemoryAllocate(uint64_t bytes, int flags)
{
  std::cout << Grid_header "SharedMemoryAllocate "<< bytes<< " MMAP anonymous implementation "<<std::endl;
  assert(_ShmSetup==1);
  assert(_ShmAlloc==0);
  //////////////////////////////////////////////////////////////////////////////////////////////////////////
  // allocate the shared windows for our group
...
void GlobalSharedMemory::SharedMemoryAllocate(uint64_t bytes, int flags)
{ 
  std::cout << Grid_header "SharedMemoryAllocate "<< bytes<< " SHMOPEN implementation "<<std::endl;
  assert(_ShmSetup==1);
  assert(_ShmAlloc==0); 
  MPI_Barrier(WorldShmComm);
  WorldShmCommBufs.resize(WorldShmSize);
